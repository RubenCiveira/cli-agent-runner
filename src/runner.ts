...
export async function sendMessage(params: SendMessageParams): Promise<SendMessageResult> {
  ...
  // ── Persist user message and create run ────────────────────────────────────
  appendMessage(db, session.id, 'user', params.message)
  ...
  return new Promise((resolve) => {
    invoke({
      ...
      onDone: (exitCode) => {
        ...
        if (assistantText) appendMessage(db, session!.id, 'assistant', assistantText)
